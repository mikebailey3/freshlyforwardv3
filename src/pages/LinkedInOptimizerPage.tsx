...
function scoreBand(score: number): { label: string; className: string } {
  if (score >= 80) return { label: 'Strong', className: 'text-success-400' }
  if (score >= 50) return { label: 'Needs Work', className: 'text-warning-400' }
  return { label: 'Weak', className: 'text-error-400' }
}
...
function SectionCard({ title, result }: { title: string; result: SectionResult }) {
  ...
  return (
    <div className="border border-border bg-surface-card p-5">
      <div className="flex items-center justify-between">
        <h3 className="font-serif text-base font-semibold text-ink">{title}</h3>
        <span className={`text-sm font-bold ${strong ? 'text-success-400' : pct >= 50 ? 'text-warning-400' : 'text-error-400'}`}>
      ...
      {result.suggestion && (
        <div className="mt-3 flex gap-2 border border-primary-700 bg-primary-950 p-3 text-sm text-primary-300">
  ...
  return (
    <MemberLayout>
      <div className="mb-6 flex items-center gap-2">
        ...
        <div>
          <h1 className="font-serif text-2xl font-semibold text-ink sm:text-3xl">LinkedIn Optimizer</h1>
      ...
      <div className="mb-6 flex items-start gap-2 border border-primary-700 bg-primary-950 px-4 py-3 text-sm text-primary-300">
      ...
      {error && (
        <div className="mb-6 border border-error-700 bg-error-950 px-4 py-3 text-sm text-error-300">
          {error}
        </div>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="border border-border bg-surface-card p-5 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1 block text-xs font-medium text-ink-muted">LinkedIn Profile URL</label>
                <input
                  type="url"
                  value={form.linkedinUrl}
                  onChange={handleChange('linkedinUrl')}
                  placeholder="https://linkedin.com/in/yourname"
                  className="w-full border border-border px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
                />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-ink-muted">Target Role</label>
                <input
                  type="text"
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-ink-muted">
                Experience Bullets <span className="font-normal text-ink-muted">(one per line)</span>
              </label>
              <textarea
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-ink-muted">
                Skills <span className="font-normal text-ink-muted">(comma-separated)</span>
              </label>
              <textarea
        ...
        <div className="lg:col-span-2 space-y-4">
          {result ? (
            <>
              <div className="flex flex-col items-center gap-2 border border-border bg-surface-card p-6">
            ...
            <div className="border border-border bg-surface-card p-8 text-center">
              <Linkedin className="mx-auto h-10 w-10 text-ink-muted" />
