  ...
  return (
    <MemberLayout>
      <div className="mb-6 flex items-center gap-2">
        ...
        <div>
          <h1 className="font-serif text-2xl font-semibold text-ink sm:text-3xl">Tools</h1>
      ...
      <div className="border border-border bg-surface-card">
        {TOOL_TILES.map((tool, i) => (
          <Link
            key={tool.label}
            to={tool.to}
            className={`flex items-center gap-4 p-5 transition-colors hover:bg-surface-hover ${i !== TOOL_TILES.length - 1 ? 'border-b border-dashed border-border' : ''}`}
          >
            <tool.icon className="h-5 w-5 flex-shrink-0 text-primary-600" />
            <div className="flex-1">
              <h3 className="font-serif text-sm font-semibold text-ink">{tool.label}</h3>
