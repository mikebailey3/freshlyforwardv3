...
describe('FreshFitBadge', () => {
  ...
  it('applies the excellent-tier style for a high score', () => {
    render(<FreshFitBadge score={90} />)
    expect(screen.getByText('FreshFit 90').className).toContain('success')
  })

  it('applies the fair-tier style for a low score', () => {
    render(<FreshFitBadge score={20} />)
    expect(screen.getByText('FreshFit 20').className).toContain('warning')
  })
})
