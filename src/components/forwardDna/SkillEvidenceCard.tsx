...
function SkillRow({ skill, onChangeState }: { skill: CareerSkill; onChangeState: SkillEvidenceCardProps['onChangeState'] }) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border pb-2">
      <span className="font-mono text-sm font-medium text-primary-400">{skill.skill_name}</span>
      <div className="flex gap-1">
        {STATE_ORDER.map((state) => (
          <button
            ...
            className={`px-2.5 py-1 text-xs font-semibold ${skill.state === state ? 'bg-primary-600 text-white' : 'border border-border text-ink-muted hover:border-primary-400'}`}
          >
            {STATE_LABELS[state]}
          </button>
        ))}
